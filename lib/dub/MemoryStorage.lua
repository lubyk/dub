...
function parse:header(header, not_lazy)
  ...
  if data.kind == 'namespace' then
    ...
    if string.match(namespace.name, '::') then
      ...
      dub.warn("Ignoring nested namespace '%s'.", namespace.name)
      return
    end
...
function parse:innernamespace(elem, header)
  ...
  if self.type ~= 'dub.MemoryStorage' or
   string.match(name, '::') then
    ...
    dub.warn("Ignoring nested namespace '%s'.", name)
    return
  end
  local namespace = dub.Namespace {
    name   = name,
...
function parse:innerclass(elem, header, not_lazy)
  ...
  if string.match(fullname, '::') then
    ...
    for i, part in ipairs(parts) do
      ...
      if not child then
        dub.warn("Ignoring class '%s'.", fullname)
...
function private.checkDoxygenVersion(data)
  ...
  if not checked_versions[str] then
    ...
    if not ok then
      dub.warn("WARNING: XML generated by Doxygen '%s'. This version of Dub was tested with versions %s.", str, lk.join(versions, ', '))
    end
  end
end
